...
def create_pie_chart(total_attempts, successful_attempts):
    labels = 'Successful Attempts', 'Unsuccessful Attempts'
    sizes = [successful_attempts, total_attempts - successful_attempts]
    colors = ['#4CAF50', '#FF6347']
    explode = (0.1, 0)  # explode the 1st slice (i.e. 'Successful Attempts')
...
# Initialize the session state attributes if they don't already exist
...
successful_attempts = st.session_state.successful_attempts
...
if feature == "Chat Assistance":
    ...
    if "quiz_initialized" not in st.session_state:
        ...
        if "quiz_answer_submitted" in st.session_state:
    ...
    if "generated_question" in st.session_state:
        ...
        st.write(f"Successful Attempts: {successful_attempts}")
        if total_attempts > 0:
            create_pie_chart(total_attempts, successful_attempts)
            score_percentage = (successful_attempts / total_attempts) * 100
            st.write(f"You have scored {score_percentage:.2f}%")
        else:
            st.write("No attempts made yet.") 
